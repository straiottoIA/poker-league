# Home Dynamic News Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Substituir os 3 cards de notícias hardcoded de 2017 da home por cards dinâmicos: última etapa encerrada, último campeão de temporada e líder atual.

**Architecture:** Task 1 adiciona `getLastWeekSummary` em `lib/queries/scores.ts`. Task 2 atualiza `app/page.tsx` para buscar os dados e montar os 3 cards dinamicamente, substituindo a constante `NEWS` estática.

**Tech Stack:** Next.js 16 (Server Component), TypeScript, Supabase, Tailwind CSS 4, funções de query existentes (`getLeaderboard`, `getHallOfFame`, `getActiveSeason`).

---

### Task 1: Adicionar `getLastWeekSummary` em scores.ts

**Files:**
- Modify: `lib/queries/scores.ts`

**Step 1: Adicionar a interface e a função ao final do arquivo**

Adicionar após a última função (`getAllSeasonScores`):
...
```bash
git add lib/queries/scores.ts
git commit -m "feat: add getLastWeekSummary query"
```

---

### Task 2: Atualizar `app/page.tsx` com news dinâmicas

**Files:**
- Modify: `app/page.tsx`

**Step 1: Substituir o conteúdo EXATO do arquivo pelo seguinte**
...
Expected: ✓ Compiled successfully — `/` deve aparecer na lista de rotas como `ƒ` (dynamic).

**Step 3: Commit e push**

```bash
git add app/page.tsx
git commit -m "feat: replace hardcoded news with dynamic cards (last week, champion, leader)"
git push
```
