# Check-in + Placements Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a placements section to the check-in page so the admin can order players by final position and have points calculated automatically.

**Architecture:** Single component change — `check-in-form.tsx` gains a second section (Colocações) that appears once at least one player is checked in. A new `lib/utils/points.ts` isolates the scoring formula. No new pages, no new API routes — reuses `upsertScores` from `lib/queries/scores.ts`.

**Tech Stack:** Next.js 16, React 19, TypeScript, Tailwind CSS 4, Supabase client.

---

> **Note on testing:** No test framework configured. Verification = `npm run build` passes + manual browser check.

---

### Task 1: `calculatePoints` utility
...
```bash
cd C:/Users/User/poker-league && git add lib/utils/points.ts && git commit -m "feat: add calculatePoints placeholder utility"
```

---

### Task 2: Placements section in `CheckInForm`
...
Replace the entire file content with the implementation below. It keeps all existing check-in logic intact and adds:
- `placements` state: `Map<playerId, position>`
- `savingScores`, `saveError`, `saveSuccess` state
- Derived: `checkedInPlayers`, `isPlacementsValid`, `duplicatePositions`
- `handleSaveScores`: calculates points for ALL players (attended + absent), calls `upsertScores`
- New JSX section 2 (Colocações) visible when `isLoggedIn && checkedIn.size >= 1`
...
Use the Write tool to overwrite `components/check-in-form.tsx` with the content above.
...
```bash
cd C:/Users/User/poker-league && git add components/check-in-form.tsx && git commit -m "feat: add placements section to check-in form with auto point calculation"
```
...
```bash
cd C:/Users/User/poker-league && git push
```
